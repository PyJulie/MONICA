    ...
    for epoch in range(configs.general.train_epochs):
        ...
        for data in tqdm.tqdm(dataloaders['train']):
            ...
            if configs.general.method == 'BBN':
                ...
                # data = ([img1, img2], labels)，inputs 是两视角列表，不能直接 .cuda()
                inputs, labels = data
                if configs.cuda.use_gpu:
                    labels = Variable(labels.cuda())
                else:
            ...
            if configs.general.method == 'MixUp' or 'GCL' in configs.general.method:
                ...
                # inputs 是 [img1, img2]（twice=True 时 dataloader 返回两个视角）
                img1, img2 = inputs[0], inputs[1]
                if configs.cuda.use_gpu:
                    img1 = img1.cuda()
                    img2 = img2.cuda()
                feature1 = model.forward_features(img1)
                feature1 = model.forward_head(feature1, pre_logits=True)
                feature2 = model.forward_features(img2)
                feature2 = model.forward_head(feature2, pre_logits=True)
                outputs = model(img1)
                conmix_args = [feature1, feature2, labels]
                train_loss = calculate_loss(configs, outputs, conmix_args, loss_functions['train'], 'train')
                correct, total = calculate_metrics_single(labels, outputs, correct, total)
